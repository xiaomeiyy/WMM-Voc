function [w0,state] = awa_resnet_deep_svr_main()
clear;clc;
% addpath('./internal/');
% addpath('Training_Algorithms/');
% addpath_folder('./internal/');
awaopt = struct([]);
awaopt =getPrmDflt(awaopt,{'Training_Algorithm',@EVL_no_loss_v3,...
		'updating_v',false,'openset',false,...
        'updating_k_l',true,'portion',0.3,...
        'ini_w',false,...
        'load_dict',@load_small_dictionary_w,...
        'load_data',@load_awa_googlenet_w_split,...
...
[x,y] = num5instance(x,y,1);

% if awaopt.pca == true
% 	[x.tr,coeff] = EVL_PCA_v2(full(x.tr),awaopt.pca_dim);
% end
...
opts.batchSize = 16;
opts.learningRates = 0.00001 * [0.9*ones(1,200) 0.00001*ones(1,10) 0.25*ones(1,5) 0.12*ones(1,5)] ;
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    [w0, res] = vl_svr(w0, im, labels, params, awa_proto) ;
